...
export default function InflacionDesglose() {
  ...
  const handleItemClick = (itemName) => {
    setSelectedCategories((prevCategories) => {
      if (prevCategories.includes(itemName)) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center">
      ...
      <div className="w-full h-full flex">
              ...
              {selectedCategories.map((category) => {
                ...
                return (
                  <Line
                    ...
                    name={category}
                    data={[
                      { name: "Diciembre", Inflación: categoryData.diciembre },
                      { name: "Enero", Inflación: categoryData.enero },
                      { name: "Febrero", Inflación: categoryData.febrero },
                    ]}
                    stroke="#1f2937"
                    dot={{
                      stroke:
        ...
        >
          <form className="w-full flex flex-col justify-around h-full text-xs">
            {data.map((item) => (
              <label key={item.name} className="cursor-pointer p-1">
                <input
                  type="checkbox"
